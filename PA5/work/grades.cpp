...
// loop to deal with different commands
// PRE: user has entered the correct number and type of arguments
void cmdLoop(Table * grades) {
   bool keepgoing = true;
   string command = "";
   while (keepgoing) {
...
void insertCmd(Table * grades) {
   ...
   int score;
   cin >> name >> score;
   bool succ = grades->insert(name, score);
   if (!succ) {
